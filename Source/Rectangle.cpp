...
//Collision Detection
	...
	bool Rectangle::AABBCollidesWith(const Rectangle& otherShape, Vector2D* contact, Vector2D* normal) const {
		...
		if (overlapping) {
			...
			if (normal != nullptr) {
				...
				*normal = locNorm.x*Vector2D(std::cos(thetaS), std::sin(thetaS));
				*normal += locNorm.y*Vector2D(-std::sin(thetaS), std::cos(thetaS));
			}

			if (contact != nullptr) {
				Vector2D locCont = (minAxH) ? Vector2D(tDir.x-uNorm.x*tExtO.x, cVal) : Vector2D(cVal, tDir.y-uNorm.y*tExtO.y);
				*contact = locCont.x*Vector2D(std::cos(thetaS), std::sin(thetaS));
				*contact += locCont.y*Vector2D(-std::sin(thetaS), std::cos(thetaS));
	...
	bool Rectangle::CollidesWith(const Circle& otherShape, Vector2D* contact, Vector2D* normal) const {
		Circle enclosure(this->GetCenter(), this->GetExtents().GetMagnitude());
		if (!enclosure.CollidesWith(otherShape, nullptr, nullptr)) return false;
		...
		if (overlapping) {
			if (normal != nullptr) {
				if (std::abs(tDir.x) <= ext.x && std::abs(tDir.y) <= ext.y) {
					...
					*normal = (minPenAxX) ? axH*tDir.x/(std::abs(tDir.x)+eps) : axV*tDir.y/(std::abs(tDir.y)+eps);
					*normal *= depth;
				}
				else {
					float depth = r - (tDir-clampPoint).GetMagnitude();
					*normal = (tDir-clampPoint);
					*normal = normal->x*axH + normal->y*axV;
	...
	bool Rectangle::CollidesWith(const Rectangle& otherShape, Vector2D* contact, Vector2D* normal) const {
		...
		if (!enclosureS.CollidesWith(enclosureO, nullptr, nullptr)) return false;
		...
		bool aligned = angularRemainder < 0.001 || std::numbers::pi/2-angularRemainder < 0.001;
		if (aligned) return this->AABBCollidesWith(otherShape, contact, normal);
		...
		else if (contact != nullptr || normal != nullptr){
		...
		if (normal != nullptr) *normal = referSelf ? minNormal*minDepth : -minNormal*minDepth;
		if (contact != nullptr) {
			...
			*contact = (incidentFace.first + incidentFace.second)/2;
		}
		return true;
	}

	bool Rectangle::CollidesWith(const JointShape& otherShape, Vector2D* contact, Vector2D* normal) const {
