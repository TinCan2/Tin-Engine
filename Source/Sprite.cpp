...
//Texture Generation
void GenerateTexture(const char* targetFile) {
	SDL_Texture* texture = IMG_LoadTexture(Sprite::boundedRenderer, targetFile);
	if (texture == nullptr) throw "Could not generate the requested texture.";
...
void Sprite::Draw(const Vector2D& position) {
	...
	SDL_RenderCopy(boundedRenderer, textureMap[this->boundTexture]->first, &sourceRect, &destRect);
}
...
void Sprite::Draw(const Vector2D& position, float rotation) {
	SDL_Rect sourceRect, destRect;
	sourceRect.x = this->x;
	sourceRect.y = this->y;
	...
	SDL_RenderCopyEx(boundedRenderer, textureMap[this->boundTexture]->first, &sourceRect, &destRect, -rotation, &rotationPoint, (SDL_RendererFlip)0);
}

void Sprite::Draw(const Vector2D& position, bool flipH, bool flipV, float rotation) {
	SDL_Rect sourceRect, destRect;
	sourceRect.x = this->x;
	sourceRect.y = this->y;
	...
	rotation = 180.0*rotation/3.141592;
	SDL_Point rotationPoint(Vector2D::UnitPixelEquivalent*this->origin->x, this->h - Vector2D::UnitPixelEquivalent*this->origin->y);

	SDL_RendererFlip flip = (SDL_RendererFlip)(flipH | (flipV<<1));
	SDL_RenderCopyEx(boundedRenderer, textureMap[this->boundTexture]->first, &sourceRect, &destRect, -rotation, &rotationPoint, flip);
}
